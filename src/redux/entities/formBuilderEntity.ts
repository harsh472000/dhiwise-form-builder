...
export const addTemplate = createAsyncThunk(
  ...
  async (data: AddTemplateType, thunkAPI) => {
    return await new Promise<TemplateType>((resolve, reject) => {
      ...
      // Retrieve templates from local storage or initialize an empty array if none exist
      const storedTemplates = getFromLocalStorage("templates");
      const allTemplates: TemplateType[] = storedTemplates ? JSON.parse(storedTemplates) : [];
